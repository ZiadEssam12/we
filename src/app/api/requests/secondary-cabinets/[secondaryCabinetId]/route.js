...
export async function GET(request, { params }) {
  ...
  try {
    const pendingRequest = await prisma.secondaryCabinet.findUnique({
      where: {
        id: requestId,
        status: {
...
export async function POST(request, { params }) {
  ...
  try {
    const existingRequest = await prisma.secondaryCabinet.findUnique({
      where: { id: requestId },
    });
    ...
    if (action === "approve") {
      ...
      await prisma.secondaryCabinet.update({
        where: { id: requestId },
        data: {
          status: "ACTIVE",
        },
