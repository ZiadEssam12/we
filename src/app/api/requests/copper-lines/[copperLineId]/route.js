...
export async function GET(request, { params }) {
  ...
  const requestId = (await params).copperLineId;

  // Check if user is authenticated (handled by middleware)
  try {
    const pendingRequest = await prisma.copperLine.findUnique({
      where: {
        id: requestId,
        status: {
...
export async function POST(request, { params }) {
  ...
  try {
    const existingRequest = await prisma.copperLine.findUnique({
      where: { id: requestId },
    });
    ...
    if (action === "approve") {
      // Approve the request by changing its status to ACTIVE
      await prisma.copperLine.update({
        where: { id: requestId },
        data: {
          status: "ACTIVE",
        },
